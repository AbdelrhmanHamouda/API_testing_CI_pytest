...
def create_post():
    """
    Functiuon to create a json POST

    :return: json 
    """
    return{
        "title": "A random string from RANDOM.ORG",
        "body": "hGYhUGKhDkDKaIlfDAfw-NETTKphpFGUAELwYxfgu-rpVGXdGauEeTEliWnwPW",
        "userId": 1
    }
...
def test_POST_response(BASE_URL):
    """
    test POST reponse
    ...
    :return: None
    """
    response = requests.post(f'{BASE_URL}', json=create_post())
    status_code = response.status_code
    is_int_id = isinstance(response.json()['id'], int)
...
def test_POST_response_to_custom_name():
    """
    test POST reponse
    ...
    """
    name = "Abdelrhman"
    json_obj = create_billpay_for(name)
    response = requests.post("http://parabank.parasoft.com/parabank/services/bank/billpay?accountId=12345&amount=500",
                             headers={"Accept": "application/json"}, json=json_obj)
    status_code = response.status_code
    assert status_code == 200
    assert response.json()["payeeName"] == name
